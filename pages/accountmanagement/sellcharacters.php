...
$add_sell_character = function ($player_id, $price_type, $price, $rk) use ($verifica_rk, $account_logged, $config, $SQL) {
    ...
    if ($vrk) {
            ...
            if ($va != NULL && $va != '' && $va != '0' && $va != 0 || !Website::getWebsiteConfig()->getValue('sell_by_gold')) {
                if ($va != $player_id || !Website::getWebsiteConfig()->getValue('sell_by_gold')) {
                    ...
                    if ($price > 0 && $price <= $price_max) {
                            ...
                            if ($valida == 0) {
                                ...
                                if ($valida > 0) {
                                   ...
                                   VALUES (:account_id, NULL , :player_id, :now, NULL);");
                                        ...
                                        if (Website::getWebsiteConfig()->getValue('sell_by_gold')) {
                                            $data = getStatus(FALSE, 'Player inserted with success.');
                                        } else {
                                            $data = getStatus(FALSE, 'Player inserted successfully. The sale through GOLD is inactive so the chosen value will be treated as coins.');
                                        }
                                        return json_encode($data);
                                    } else {
                                        $data = getStatus(TRUE, 'The player you tried to sell does not belong to this account.');
                                        return json_encode($data);
                                    }
                                } else {
                                    $data = getStatus(TRUE, 'This player is below the level necessary for sale.');
                                    return json_encode($data);
                                }
                            } else {
                        ...
                        $data = getStatus(TRUE, "The price should be between 1 and " . number_format($price_max, 0, ',', '.'));
                        return json_encode($data);
                    }
                } else {
                    $data = getStatus(TRUE, "You can not sell your player that is receiving money from your purchases.");
                    return json_encode($data);
                }
            } else {
                $data = getStatus(TRUE, "You can not sell without first selecting a player to receive the value of your sales made in the currencies.");
                return json_encode($data);
            }
        ...
        $data = getStatus(TRUE, "The RK (recovery key) entered is not compatible with the RK of the online account.");
        return json_encode($data);
    }
};
...
//    var_dump($valida->rowCount(), $valida->fetchAll());
            if ($valida->rowCount() > 0) {
                ...
                $data = getStatus(FALSE, 'Player removed from the sale successfully');
                return json_encode($data);
            } else {
                $data = getStatus(TRUE, 'This player could not be deleted or is not for sale or does not belong to your account.');
                return json_encode($data);
            }
                ...
                if ($valida_acc->rowCount() == 0) {
                    ...
                    if ($char_id != NULL) {
                        ...
                        if ($account_logged->getID() != $s->getAccountID()) {
                            $data = getStatus(TRUE, 'You do not have permission to see this.');
                            return json_encode($data);
                        }
                    ...
                    if ($p[0]['price_type'] == 0) {
                        ...
                        if ($price <= $saldo) {
                            ...
                            $data = getStatus(FALSE, 'You bought this character successfully.');
                            return json_encode($data);
                        } else {
                            $data = getStatus(TRUE, 'You do not have enough balance to make this purchase.');
                            return json_encode($data);
                        }
                    } else {
                        if ($char_id > 0) {
                            ...
                            if ($price <= $saldo) {
                                ...
                                if ($verifica_logado->rowCount() == 0) {
                                    ...
                                    if ($verifica_banco_logado->rowCount() == 0) {
                                        ...
                                        $data = getStatus(TRUE, 'The character bank of the buyer can not be online. Please request that you log out and try again.');
                                        return json_encode($data);
                                    }
                                } else {
                                    $data = getStatus(TRUE, 'Your character can not be connected when making this purchase. Please, disconnect and try again.');
                                    return json_encode($data);
                                }
                            } else {
                                $data = getStatus(TRUE, 'You do not have enough balance to make this purchase.');
                            ...
                            $data = getStatus(TRUE, 'For that operation you need to select a character whose which balance balance will be used for the purchase.');
                            return json_encode($data);
                        }
                    }
                } else {
                    $data = getStatus(TRUE, 'You can not buy your own character.');
                    return json_encode($data);
                }
            } else {
                $data = getStatus(TRUE, "You do not have permission to see this.");
    ...
    if ($req) {
        if ($id > 0) {
            ...
            if (count($val) > 0) {
                ...
                $q->execute();
                $data = getStatus(FALSE, 'You have updated the player successfully. In the case of selling some character through Gold the golds will be delivered to him.');
                return json_encode($data);
            } else {
...
<p style='text-align: center'><b><font color=red>Attention!</font></b></p>
<p style='text-align: center'>A percentage of (" . Website::getWebsiteConfig()->getValue('percent_sellchar_sale') . "%) will be charged for each sale made.</p>
<p>Before making a sale you need to choose a player to receive the gold from the sales - This player can not be sold.</p>";
...
<p>Check correctly if the character you are selling is what you really want to sell, and also the price of it, once you post the sale you can just give up the sale a day later. Below is the list of characters you can sell.</p>";
    ...
    $query = $SQL->prepare("SELECT * FROM players WHERE account_id = ".$account_logged->getId());
    $query->execute([]);
    $p = $query->fetchAll();
    $i = 0;
    if (!empty($p)) {
...
';
        ...
        $main_content .= '
                    <tr style="background-color:#F1E0C6;">
                        <td colspan="6">You still do not have characters in your account.</td>
                    </tr>';
    }
...
<script>
    ...
    return false;
        ...
        $main_content .= '
                    <tr style="background-color:#F1E0C6;">
                        <td colspan="6">You do not have characters for sale.</td>
                    </tr>';
    }
