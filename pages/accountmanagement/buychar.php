        ...
        ";
            ...
            $verifica_item_id = function ($pid) use ($player_information) {
                if ($player_information->getItems()->getItem($pid)[array_keys($player_information->getItems()->getItem($pid))[0]]->data['itemtype'] == null) {
                    return '<td style="background-color: #d4c0a1; text-align: center;"><img src="./images/shop/items/' . $pid . '.gif" width="44" higth="44"></td>';
                } else {
                    $item_id = $player_information->getItems()->getItem($pid)[array_keys($player_information->getItems()->getItem($pid))[0]]->data['itemtype'];
                    return '<td style="background-color: #d4c0a1; text-align: center;"><img src="./images/shop/items/' . $item_id . '.gif" width="44" higth="44"></td>';
                }
            };
            ...
            if ($outfits != false) {
                foreach ($outfits as $value) {
                    $main_content .= "<img src='http://comandosot.com/outfits/animoutfit.php?id={$value['looktype']}&addons={$value['addon']}&head={$player_information->getLookHead()}&body={$player_information->getLookBody()}&legs={$player_information->getLookLegs()}&feet={$player_information->getLookFeet()}&mount=0'>";
                }
            } else {
            ...
            if ($mounts != false) {
                foreach ($mounts as $value) {
                    $main_content .= "<img src='http://comandosot.com/outfits/animoutfit.php?id={$value['clientid']}'>";
                }
            } else {
            ...
            if ($player_information->getDepotItems() != null) {
                ...
                foreach ($depotitems as $dep) {
                    ...
                    $main_content .= "<div class='depot-item'><img src='./images/shop/items/{$dep["itemtype"]}.gif'><br/>Name:" . (isset($itemm['article']) ? $itemm['article'] : '') . " " . $itemm['name'] . "<br/>Qnt:{$dep["real_count"]}</div>";
                }
                $main_content .= "</div>";
            } else {
                $main_content .= "<p>This player does not have any items.</p>";
            }
            $main_content .= '</td></tr>
                                                                        </tbody>
                                                                    </table>
            ...
            if ($items != null) {
                ...
                foreach ($items as $item) {
                    ...
                    $main_content .= "<div class='depot-item'><img src='./images/shop/items/{$item["itemtype"]}.gif'><br/>Name:" . (isset($itemm['article']) ? $itemm['article'] : '') . " " . $itemm['name'] . "<br/>Qnt:{$item["real_count"]}</div>";
                }
                $main_content .= "</div>";
            } else {
                $main_content .= "<p>This player does not have any items.</p>";
...
<tr>
    ";
            $main_content .= "
          <td><img class='Outfit' src='http://comandosot.com/outfits/animoutfit.php?id={$player_info['looktype']}&addons={$player_info['lookaddons']}&head={$player_info['lookhead']}&body={$player_info['lookbody']}&legs={$player_info['looklegs']}&feet={$player_info['lookfeet']}&mount=" . ($mount_id == null ? 0 : $mount_id) . "' alt='' name=''></td>
        ";
