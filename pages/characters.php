    ...
    if ($player->isLoaded()) {
        ...
        $verifica_item_id = function ($pid) use ($player) {
            ...
            if ($player->getItems()->getItem($pid)[$kalabok]->data['itemtype'] == NULL) {
                return '<td style="background-color: #d4c0a1; text-align: center;"><img src="./images/shop/items/' . $pid . '.gif" class="CharItems"></td>';
            } else {
                $item_id = $player->getItems()->getItem($pid)[$kalabok]->data['itemtype'];
                return '<td align="center" style="background-color: #D4C0A1;"><img src="./images/shop/items/' . $item_id . '.gif" class="CharItems"></td>';
            }
        };
...
//            $main_content .="</div><br/>";
        ...
        //quest list
        if (isset($config['site']['tasks']) && is_array($config['site']['tasks']) && count($config['site']['tasks']) > 0) {
            ...
            $main_content .= '<div class="TableContainer" >';
            $main_content .= $make_content_header("Tasks");
            $main_content .= $make_table_header('Table3', '', TRUE);
            $number_of_quests = 0;
            foreach ($config['site']['tasks'] as $questName => $storageID) {
                $bgcolor = (($number_of_rows++ % 2 == 1) ? $config['site']['darkborder'] : $config['site']['lightborder']);
                $number_of_quests++;
                $main_content .= '<TR ><TD WIDTH=95%>' . $questName . '</TD>';
                if ($player->getStorage($storageID) === NULL) {
